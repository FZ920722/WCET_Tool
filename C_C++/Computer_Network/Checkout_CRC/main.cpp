...
int main(int argc, char** argv) {
	unsigned short return_ret = 0;
	unsigned char te2st[12] = {	0x11,0x11, 0x11,
								0x12, 0x11,0x13, 
								0x11,0x21, 0x11,
								0x22, 0x11,0x23 };
	unsigned char test[2] = {	0x11, 0x12};
	return_ret = IOT_CRC16_CCITT_FALSE(test,sizeof(test));
	printf("IOT_CRC16_CCITT_FALSE 		= 0x%4x\n", return_ret);
	return_ret = IOT_CRC16_CCITT(test,sizeof(test));
	printf("IOT_CRC16_CCITT 		= 0x%4x\n", return_ret);
	
	return_ret = IOT_CRC16_MODBUS(test,sizeof(test));
	printf("IOT_CRC16_MODBUS 		= 0x%4x\n", return_ret);
	return_ret = IOT_CRC16_XMODEM(test,sizeof(test));
	printf("IOT_CRC16_XMODEM 		= 0x%4x\n", return_ret);
	return_ret = IOT_CRC16_YMODEM(test,sizeof(test));
	printf("IOT_CRC16_YMODEM 		= 0x%4x\n", return_ret);
	return 0;
}
